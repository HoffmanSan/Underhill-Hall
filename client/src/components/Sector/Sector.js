...
export default function Sector({sectorRef, seatsQuantity, takenSeats, pickedSeats, handleClick}) {
  ...
  return (
    <div className={`sector sector-${sectorRef}`}>
      ...
      <div className="sector-name">
        <h2>SECTOR {sectorRef}</h2>
      </div>

      {sectorSeats.map(seat => (
        <button
          key={`${seat.sectorRef}${seat.seatNumber}`}
          // If the user picked this seat, give it a class of 'Picked' || If this seat is already booked, give it a class of 'Taken'
          className={(pickedSeats.includes(`${seat.sectorRef}${seat.seatNumber}`)) ? 'Picked' : '' || (takenSeats.includes(`${seat.sectorRef}${seat.seatNumber}`)) ? 'Taken' : ''}
